...
with open(f'{os.environ.get("ULANMEDIAAPP")}/data/c_widgets_for_one_p_widget_for_one_campaign/{volid}_{p_widget}_{date_range}_c_widgets_for_one_p_widget_for_one_campaign_dataset.json', 'r') as file:
...
c1 = df["classification"] == sys.argv[4]
result1 = df[c1]

c2 = df["status"] == sys.argv[5]
result2 = df[c2]

c3 = df["global_status"] == sys.argv[6]
result3 = df[c3]

c4 = df["cost"] >= float(sys.argv[7])
result4 = df[c4]

c5 = df["profit"] <= -1 * float(sys.argv[8])
result5 = df[c5]

c6 = df["is_bad_and_included"] == True 
result6 = df[c6]

conditions_args = [sys.argv[9], sys.argv[10], sys.argv[11], sys.argv[12], sys.argv[13], sys.argv[14]]
