...
def create_months_for_one_ad_for_all_campaigns_dataset(token, start_date, end_date, ad_image):
    try:
        ...
        while (end - start).days >= 30:
            ...
            url = f"https://api.voluum.com/report?from={temp_start}T00:00:00Z&to={temp_end}T00:00:00Z&tz=America%2FLos_Angeles&filter={ad_image}&conversionTimeMode=VISIT&currency=USD&sort=month&direction=desc&columns=month&columns=customVariable5&columns=visits&columns=conversions&columns=revenue&columns=cost&columns=profit&columns=cv&columns=roi&columns=epv&groupBy=month&groupBy=custom-variable-5&offset=0&limit=1000&include=ACTIVE&filter1=traffic-source&filter1Value=37bbd390-ed90-4978-9066-09affa682bcc"
        ...
        if (end - start).days > 0:
            ...
            url = f"https://api.voluum.com/report?from={start}T00:00:00Z&to={end}T00:00:00Z&tz=America%2FLos_Angeles&filter={ad_image}&conversionTimeMode=VISIT&currency=USD&sort=month&direction=desc&columns=month&columns=customVariable5&columns=visits&columns=conversions&columns=revenue&columns=cost&columns=profit&columns=cv&columns=roi&columns=epv&groupBy=month&groupBy=custom-variable-5&offset=0&limit=1000&include=ACTIVE&filter1=traffic-source&filter1Value=37bbd390-ed90-4978-9066-09affa682bcc"
            res = requests.get(url, headers = {"cwauth-token": token}).json()
            for row in res["rows"]:
                month = row["month"]
                clicks = row["visits"]
        ...
        with open(f"{os.environ.get('ULANMEDIAAPP')}/data/months_for_one_ad_for_all_campaigns/{ad_image}_months_for_one_ad_for_all_campaigns_dataset.json", "w") as file:
            json.dump(months, file)

        return json.dumps(months)
    except:
        print("Failed - email sent")
        send_email("brianshaller@gmail.com", "Failed - create_months_for_one_ad_for_all_campaigns_dataset()", "Failed - create_months_for_one_ad_for_all_campaigns_dataset()")
        sys.exit()
