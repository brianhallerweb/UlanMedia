...
def create_months_for_one_p_widget_for_one_campaign_dataset(token, start_date, end_date, p_widget_id, campaign_id):
    try:
        ...
        with open(f"{os.environ.get('ULANMEDIAAPP')}/data/months_for_one_p_widget_for_one_campaign/{p_widget_id}_{campaign_id}_months_for_one_p_widget_for_one_campaign_dataset.json", "w") as file:
            json.dump(months, file)

        return json.dumps(months)
    except:
        print("Failed - email sent")
        send_email("brianshaller@gmail.com", "Failed - create_months_for_one_p_widget_for_one_campaign_dataset()", "Failed - create_months_for_one_p_widget_for_one_campaign_dataset()")
        sys.exit()
