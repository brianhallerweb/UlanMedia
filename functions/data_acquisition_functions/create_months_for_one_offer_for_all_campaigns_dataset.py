...
def create_months_for_one_offer_for_all_campaigns_dataset(token, start_date, end_date, offer_name):
    try:
        ...
        with open(f"{os.environ.get('ULANMEDIAAPP')}/data/months_for_one_offer_for_all_campaigns/{offer_name}_months_for_one_offer_for_all_campaigns_dataset.json", "w") as file:
            json.dump(months, file)

        return json.dumps(months)
    except:
        print("Failed - email sent")
        send_email("brianshaller@gmail.com", "Failed - create_months_for_one_offer_for_all_campaigns_dataset()", "Failed - create_months_for_one_offer_for_all_campaigns_dataset()")
        sys.exit()
